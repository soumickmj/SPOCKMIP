...
def warp_image(image, displacement, multi=False):
    ...
    if multi:
        ...
        warped_image = F.grid_sample(image.unsqueeze(0).unsqueeze(0),
                                     grid)  # [B, C, D, H, W], unsqueeze to give batch and channel dimension
...
class _ParameterizedKernelTransformation(_KernelTransformation):
    ...
    def _initialize(self):
        ...
        if self.rnd_grid_params is None:
            ...
            self.trans_parameters = Parameter(self.get_coarse_field(cp_grid, self.rnd_grid_params['max_displacement'],
                                                                    self.rnd_grid_params['num_locked_borders']))
...
class ParameterizedBsplineTransformation(_ParameterizedKernelTransformation):
    def __init__(self, image_size, sigma, rnd_grid_params=None, diffeomorphic=False, order=2, dtype=th.float32,
                 device='cpu'):
        super(ParameterizedBsplineTransformation, self).__init__(image_size, rnd_grid_params, diffeomorphic, dtype,
                                                                 device)
